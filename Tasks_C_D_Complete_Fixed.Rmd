...
# Data preprocessing with error handling for date parsing
tryCatch({
  data$date_parsed <- dmy_hm(data$date)
  data$user_created_parsed <- dmy_hm(data$user_created_at)
  
  # Check for parsing failures
  na_dates <- sum(is.na(data$date_parsed))
  if (na_dates > 0) {
    cat("Warning:", na_dates, "dates could not be parsed\n")
  }
}, error = function(e) {
  cat("Error in date parsing:", conditionMessage(e), "\n")
  cat("Expected format: day/month/year hour:minute (e.g., '23/07/2021 10:30')\n")
})
...
### (a) R Code:
```{r question-c3}
# Define outlier thresholds as constants for transparency
...
social_analysis <- data %>%
  ...
  filter(user_followers < MAX_FOLLOWERS_THRESHOLD & user_friends < MAX_FRIENDS_THRESHOLD) %>%
...
### (a) R Code:
```{r question-c5b}
# Define spammer detection thresholds as constants
SPAM_TWEETS_PER_DAY_THRESHOLD <- 50
SPAM_LOW_ENGAGEMENT_THRESHOLD <- 1
SPAM_DUPLICATE_RATIO_THRESHOLD <- 0.3
SPAM_RETWEET_RATIO_THRESHOLD <- 0.8
SPAM_SCORE_MINIMUM <- 2
...
spammer_analysis <- data %>%
  ...
  mutate(
    spammer_score = (tweets_per_day > SPAM_TWEETS_PER_DAY_THRESHOLD) + 
                   (avg_engagement < SPAM_LOW_ENGAGEMENT_THRESHOLD) + 
                   (duplicate_text_ratio > SPAM_DUPLICATE_RATIO_THRESHOLD) + 
                   (retweet_ratio > SPAM_RETWEET_RATIO_THRESHOLD)
  ) %>%
  filter(spammer_score >= SPAM_SCORE_MINIMUM) %>%  # Users meeting 2+ spam criteria
...
cat("\n1. High posting frequency (>", SPAM_TWEETS_PER_DAY_THRESHOLD, "tweets/day)")
cat("\n2. Low engagement (<", SPAM_LOW_ENGAGEMENT_THRESHOLD, "avg engagement)")
cat("\n3. High duplicate content (>", SPAM_DUPLICATE_RATIO_THRESHOLD * 100, "% duplicate text)")
cat("\n4. Excessive retweeting (>", SPAM_RETWEET_RATIO_THRESHOLD * 100, "% retweets)")
cat("\nUsers meeting", SPAM_SCORE_MINIMUM, "+ criteria are flagged as potential spammers.")
```
...
### (a) R Code:
```{r question-c6a}
# Define minimum sentiment threshold for statistical reliability
...
sentiment_analysis <- data %>%
  ...
  filter(total_sentiment >= MIN_SENTIMENT_WORDS) %>%  # Minimum threshold for reliability
...
### (c) Explanation:
This analysis uses the NRC Word-Emotion Association Lexicon to classify tweet sentiment based on emotional word content. The methodology tokenizes text, matches words against the sentiment lexicon, and calculates user-level sentiment ratios. The minimum threshold of 20 sentiment-bearing words ensures statistical reliability by filtering users with insufficient sentiment content. Users with extreme negative ratios may be expressing dissatisfaction, criticism, or engaging in controversy-driven content during the Olympics, which could indicate different engagement strategies or genuine emotional responses to events.
...
tryCatch({
  ...
  # Calculate mtry parameter using standard formula for classification
  n_features <- ncol(train_prepared) - 1
  mtry_baseline <- floor(sqrt(n_features))
  ...
  cat("Features used:", n_features, "| mtry:", mtry_baseline, "\n")
...
if (exists("model_rf_baseline")) {
  ...
  tryCatch({
    ...
    # Calculate mtry for reduced feature set
    n_features_reduced <- ncol(train_reduced) - 1
    mtry_reduced <- floor(sqrt(n_features_reduced))
    ...
    cat("Reduced model - Features:", n_features_reduced, "| mtry:", mtry_reduced, "\n")
...
tryCatch({
  ...
  # Calculate enhanced mtry parameter (1.5x standard for better feature exploration)
  n_features <- ncol(train_prepared) - 1
  mtry_enhanced <- ceiling(sqrt(n_features) * 1.5)
  ...
  cat("Enhanced RF - Features:", n_features, "| mtry:", mtry_enhanced, "| trees: 1000\n")
